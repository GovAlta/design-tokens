    ...
    formatter: function ({ dictionary, options, file }) {
      ...
      try {
        ...
        if (fs.existsSync(darkTokensPath)) {
          // Build dark mode dictionary
          const darkSD = StyleDictionary.extend({
            source: [darkTokensPath],
            platforms: {
              css: {
                prefix: options.prefix || 'goa',
                transforms: options.transforms || StyleDictionary.transformGroup.css,
              }
            }
          });
          
          const darkDictionary = darkSD.properties;
          const darkTokens = [];
