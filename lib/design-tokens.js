...
function generate(outputPath) {
  ...
  const isExperimentalToken = (token) => {
    if (typeof token?.filePath !== "string") return false;
    const segment = `${path.sep}experimental${path.sep}`;
    return token.filePath.includes(segment);
  };
    ...
    StyleDictionary.extend({
      ...
      platforms: {
        experimentalCss: {
          ...
          files: [
            {
              destination: "experimental-tokens.css",
              format: "css/variables",
              filter: isExperimentalToken,
              options: {
                outputReferences: true,
                selector: ".v2-experimental-tokens",
              },
            },
          ],
        },
